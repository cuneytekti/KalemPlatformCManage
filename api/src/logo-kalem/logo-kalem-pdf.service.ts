...
export class LogoKalemPdfService {
  ...
  html(detail: LogoKalemDetail): string {
    ...
    populated.forEach((section) => {
      ...
      chunks.forEach((lines, index) => pages.push(this.page(detail, `${section.title}${index ? ` (${index + 1})` : ''}`, this.table(lines, section.currency, t) + (index === chunks.length - 1 ? `<div class="section-total"><span>${t.total}</span><strong>${this.amount(section.netTotal, section.currency, lang)}</strong></div>` : ''), number)));
    });
  ...
  private table(lines: LogoKalemDetail['sections'][number]['lines'], currency: string, t: typeof copy[Language]): string { return `<table><thead><tr><th>${t.product}</th><th>${t.qty}</th><th>${t.unit}</th><th>${t.price}</th><th>${t.discount}</th><th>${t.total}</th></tr></thead><tbody>${lines.map((l) => `<tr><td><strong>${this.e(l.name)}</strong>${l.description ? `<small>${this.e(l.description)}</small>` : ''}${l.location ? `<em>${this.e(l.location)}</em>` : ''}</td><td>${this.e(l.quantity)}</td><td>${this.e(l.unit)}</td><td>${this.amount(l.unitPrice, currency, 'tr')}</td><td>${this.amount(l.discountTotal, currency, 'tr')}</td><td><strong>${this.amount(l.netTotal, currency, 'tr')}</strong></td></tr>`).join('')}</tbody></table>`; }
