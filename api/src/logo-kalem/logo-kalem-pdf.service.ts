...
export class LogoKalemPdfService {
  ...
  html(detail: LogoKalemDetail): string {
    ...
    const summaryAdjustments = detail.adjustments.filter((item) => item.target === 'MAIN');
    pages.push(this.page(detail, t.summary, `${this.investmentSummary(detail, lang, t)}${summaryAdjustments.length ? `<h2>${this.e(adjustmentTitle)}</h2><div class="adjustments">${summaryAdjustments.map((a) => `<div><span>${this.e(a.label)}</span><strong>${this.amount(a.amount, this.currency(detail, [a.target]), lang)}</strong></div>`).join('')}</div>` : ''}`, number));
