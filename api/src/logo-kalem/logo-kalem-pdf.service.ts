...
type PricingLine = LogoKalemDetail['sections'][number]['lines'][number];
...
export class LogoKalemPdfService {
  ...
  html(detail: LogoKalemDetail): string {
    ...
    const storeLines = this.storeCostLines(detail);
    const firstStoreLines = storeLines.slice(0, 6);
    pages.push(this.page(detail, t.summary, `${this.investmentSummary(detail, lang, t)}${storeLines.length ? this.storeCostBlock(firstStoreLines, storeLines.length <= 6, storeLines, this.currency(detail, ['MAIN']), lang, t) : this.storeCostEmpty(t)}`, number));
    if (storeLines.length > 6) {
      const continuationChunks = this.chunk(storeLines.slice(6), 9);
      continuationChunks.forEach((lines, index) => pages.push(this.page(detail, `${t.storeCost} (${index + 2})`, this.storeCostBlock(lines, index === continuationChunks.length - 1, storeLines, this.currency(detail, ['MAIN']), lang, t, false), number)));
    }
  ...
  private table(lines: LogoKalemDetail['sections'][number]['lines'], currency: string, lang: Language, t: typeof copy[Language]): string { return `<table><thead><tr><th>${t.product}</th><th>${t.qty}</th><th>${t.unit}</th><th>${t.price}</th><th>${t.discount}</th><th>${t.total}</th></tr></thead><tbody>${lines.map((l) => l.pricingMode === 'LICENSE_PERCENT' ? `<tr><td><strong>${this.e(l.name)}</strong>${l.description ? `<small>${this.e(l.description)}</small>` : ''}</td><td>1</td><td>${this.e(l.unit)}</td><td><small>${this.e(t.licenseBase)}</small><strong>${this.amount(l.calculationBase, currency, lang)}</strong></td><td><small>${this.e(t.lemRate)}</small><strong class="discount-rate">${this.percent(Number(l.ratePercent || 0), lang)}</strong></td><td><small>${this.e(t.annualLem)}</small><strong>${this.amount(l.netTotal, currency, lang)}</strong></td></tr>` : `<tr><td><strong>${this.e(l.name)}</strong>${l.description ? `<small>${this.e(l.description)}</small>` : ''}${l.location ? `<em>${this.e(l.location)}</em>` : ''}</td><td>${this.e(l.quantity)}</td><td>${this.e(l.unit)}</td><td>${this.amount(l.unitPrice, currency, lang)}</td><td><strong class="discount-rate">${this.percent(this.lineDiscountRate(l), lang)}</strong><small>${this.amount(l.discountTotal, currency, lang)}</small></td><td><strong>${this.amount(l.netTotal, currency, lang)}</strong></td></tr>`).join('')}</tbody></table>`; }
  ...
  private sectionSummary(section: LogoKalemDetail['sections'][number], lang: Language, t: typeof copy[Language]): string {
    const percentageLem = section.lines.find((line) => line.pricingMode === 'LICENSE_PERCENT');
    if (percentageLem) return `<div class="section-summary lem-summary"><div><span>${t.licenseBase}</span><strong>${this.amount(percentageLem.calculationBase, section.currency, lang)}</strong></div><div><span>${t.lemRate}</span><strong>${this.percent(Number(percentageLem.ratePercent || 0), lang)}</strong></div><div class="accent"><span>${t.annualLem}</span><strong>${this.amount(percentageLem.netTotal, section.currency, lang)}</strong></div></div>`;
  ...
  private investmentSummary(detail: LogoKalemDetail, lang: Language, t: typeof copy[Language]): string {
    const aggregate = (type: 'MAIN' | 'SERVICE') => detail.sections.filter((section) => section.type === type).reduce((row, section) => ({ list: row.list + Number(section.subtotal || 0), discount: row.discount + Number(section.discountTotal || 0), net: row.net + Number(section.netTotal || 0), tax: row.tax }), { list: 0, discount: 0, net: 0, tax: 0 });
    const rows = [{ label: t.licenses, ...aggregate('MAIN') }, { label: t.services, ...aggregate('SERVICE') }];
    const allocate = (amount: number, key: 'discount' | 'net' | 'tax') => { const base = rows.reduce((sum, row) => sum + row.net, 0); if (base <= 0) return; let assigned = 0; rows.forEach((row, index) => { const share = index === rows.length - 1 ? amount - assigned : Math.round(amount * row.net / base * 100) / 100; assigned += share; row[key] += share; if (key === 'discount') row.net -= share; }); };
    detail.adjustments.filter((item) => item.target === 'MAIN').forEach((item) => { const amount = Number(item.amount || 0); if (item.type === 'DISCOUNT') allocate(amount, 'discount'); else if (item.type === 'OTHER') allocate(amount, 'net'); else if (item.type === 'TAX') allocate(amount, 'tax'); });
    const currency = this.currency(detail, ['MAIN', 'SERVICE']);
    const body = rows.map((row) => `<tr><td><strong>${this.e(row.label)}</strong></td><td>${this.amount(row.list.toFixed(2), currency, lang)}</td><td>${this.amount(row.discount.toFixed(2), currency, lang)}</td><td>${this.amount(row.net.toFixed(2), currency, lang)}</td><td>${this.amount(row.tax.toFixed(2), currency, lang)}</td><td><strong>${this.amount((row.net + row.tax).toFixed(2), currency, lang)}</strong></td></tr>`).join('');
    const netTotal = rows.reduce((sum, row) => sum + row.net, 0);
    const total = rows.reduce((sum, row) => sum + row.net + row.tax, 0);
    return `<table class="investment-table"><thead><tr><th>${t.pricing}</th><th>${t.listTotal}</th><th>${t.discountAmount}</th><th>${t.netTotal}</th><th>${t.vat}</th><th>${t.amountWithVat}</th></tr></thead><tbody>${body}</tbody></table><div class="investment-total"><span>${t.offerTotal}</span><div class="investment-total-values"><div><small>${t.offerExclVat}</small><strong>${this.amount(netTotal.toFixed(2), currency, lang)}</strong></div><div><small>${t.offerInclVat}</small><strong>${this.amount(total.toFixed(2), currency, lang)}</strong></div></div></div><div class="recurring-note"><strong>${t.monthlyService}: ${this.amount(detail.revision.maintenanceTotal, this.currency(detail, ['MAINTENANCE']), lang)}</strong><strong>${t.lem}: ${this.amount(detail.revision.lemTotal, this.currency(detail, ['LEM']), lang)}</strong><p>${this.e(t.recurringExcluded)}</p></div>`;
  }
  ...
  private css() { return `@page{size:A4;margin:0}*{box-sizing:border-box}body{margin:0;background:#dbe4e8;font-family:Arial,sans-serif;color:#172b40}.page{width:210mm;height:297mm;background:#fff;page-break-after:always;position:relative;overflow:hidden;padding:18mm 17mm 15mm}.page:last-child{page-break-after:auto}header{height:12mm;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #dce6e8;color:#64808b;font-size:9px;letter-spacing:2px}header img{width:13mm;height:13mm;object-fit:contain}main{padding-top:12mm}.title>span,.eyebrow{font-size:9px;color:#149b97;letter-spacing:2px;font-weight:700}.title h1{font-size:28px;margin:5px 0 18px;color:#102c47}.cover{background:#102c47;color:#fff}.cover header{border-color:#31516b;color:#87d7d3}.cover footer{color:#9bb0bf;border-color:#31516b}.cover-content{margin-top:32mm;width:88%}.cover h1{font-size:44px;line-height:1.08;color:#fff;margin:12px 0}.lead{font-size:19px;color:#b7d0db}.cover-mark{position:absolute;width:105mm;height:105mm;border:25mm solid #158f8d;border-radius:50%;right:-45mm;top:55mm;opacity:.35}.cover-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:38mm}.cover-grid div{border-top:1px solid #456177;padding-top:10px}.cover-grid small{display:block;color:#84a2b2;font-size:9px;text-transform:uppercase;letter-spacing:1px}.cover-grid strong{display:block;margin-top:6px;font-size:14px}.intro-grid{display:grid;grid-template-columns:1.6fr 1fr;gap:25px}.statement{font-size:18px;line-height:1.55}.meta-card,.callout{background:#edf6f5;border-left:4px solid #149b97;padding:18px}.meta-card span{display:block;font-size:9px;text-transform:uppercase;color:#64808b;margin:9px 0 3px}.meta-card strong{font-size:13px}.section-block{margin-top:25px}.section-block h2,h2{color:#102c47}.callout{margin-top:22px}.large-copy{font-size:19px;line-height:1.6}.value-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:28px}.value-grid div{border:1px solid #dce6e8;border-radius:9px;padding:18px;min-height:82px}.value-grid b{color:#149b97;margin-right:12px}.brand-band{margin-top:35px;background:#102c47;color:#fff;padding:24px;display:flex;justify-content:space-between}.brand-band span{color:#89d8d4}table{width:100%;border-collapse:collapse;font-size:9px}th{text-align:left;background:#102c47;color:#fff;padding:9px 7px}td{padding:9px 7px;border-bottom:1px solid #e3eaec;vertical-align:top}td small,td em{display:block;color:#667b88;margin-top:3px;font-size:8px}td em{color:#149b97}.discount-rate{display:block;color:#102c47}.section-summary{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:12px}.section-summary.lem-summary{grid-template-columns:repeat(3,1fr)}.section-summary>div{min-height:55px;padding:10px;background:#edf6f5;border:1px solid #dce6e8;border-radius:7px}.section-summary span,.section-summary strong{display:block}.section-summary span{min-height:18px;color:#64808b;font-size:7px;font-weight:700;letter-spacing:.4px;text-transform:uppercase}.section-summary strong{margin-top:4px;font-size:10px}.section-summary .accent{color:#fff;background:#149b97;border-color:#149b97}.section-summary .accent span{color:#dff8f6}.investment-table th,.investment-table td{font-size:8px}.investment-table th:first-child,.investment-table td:first-child{width:18%}.investment-total{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-top:15px;padding:15px 20px;color:#fff;background:#149b97;border-radius:9px}.investment-total>span{font-size:10px;font-weight:700;text-transform:uppercase}.investment-total-values{display:grid;grid-template-columns:1fr 1fr;gap:22px;text-align:right}.investment-total-values div+div{padding-left:22px;border-left:1px solid rgba(255,255,255,.35)}.investment-total-values small,.investment-total-values strong{display:block}.investment-total-values small{margin-bottom:4px;color:#dff8f6;font-size:7px;font-weight:700;text-transform:uppercase}.investment-total-values strong{font-size:17px}.recurring-note{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:14px;padding:16px;background:#f3f7f8;border-left:4px solid #102c47}.recurring-note strong{font-size:10px}.recurring-note p{grid-column:1/-1;margin:2px 0 0;color:#64808b;font-size:9px}.store-cost-block{margin-top:20px}.store-cost-block h2{margin:0 0 10px;font-size:18px}.store-cost-detail th,.store-cost-detail td,.store-cost-summary th,.store-cost-summary td{font-size:8px}.store-cost-detail th:first-child,.store-cost-detail td:first-child{width:28%}.store-cost-summary{margin-top:12px;border:1px solid #cfe1e2}.store-cost-summary th{text-align:center;background:#edf6f5;color:#102c47}.store-cost-summary td{text-align:center;font-size:11px;background:#f8fbfb}.store-cost-summary td:last-child{color:#fff;background:#149b97}.store-cost-empty{padding:20px;color:#64808b;background:#f3f7f8;border-left:4px solid #149b97;border-radius:0 8px 8px 0;font-size:10px;line-height:1.5}.terms{display:grid;grid-template-columns:1fr 1fr;gap:12px}.terms>div{border:1px solid #dce6e8;padding:16px;border-radius:8px}.terms span{font-size:10px;color:#149b97;font-weight:700;text-transform:uppercase}.terms p{font-size:11px;line-height:1.5}.appendix-image{display:block;width:100%;height:195mm;object-fit:contain;object-position:center top}.appendix-image.certificates{height:192mm}.source-note{margin:2mm 0 0;color:#758a95;font-size:8px;text-align:center}.muted{color:#758a95;font-size:10px;margin-top:20px}.empty{height:150mm;display:grid;place-items:center;color:#91a4ad}footer{position:absolute;left:17mm;right:17mm;bottom:8mm;padding-top:4mm;border-top:1px solid #dce6e8;display:flex;justify-content:space-between;color:#80939c;font-size:7px}`; }
}
