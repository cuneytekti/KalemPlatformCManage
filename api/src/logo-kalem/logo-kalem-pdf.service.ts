...
const copy = {
  tr: {
    ...
    licenses: 'Lisans Bedelleri', services: 'Hizmet Bedelleri', monthlyService: 'Aylık Hizmet', vat: 'KDV', amountWithVat: 'KDV Dâhil Toplam', offerTotal: 'Teklif Genel Toplamı',
  ...
  az: {
    ...
    licenses: 'Lisenziya Məbləğləri', services: 'Xidmət Məbləğləri', monthlyService: 'Aylıq Xidmət', vat: 'ƏDV', amountWithVat: 'ƏDV Daxil Cəm', offerTotal: 'Təklifin Ümumi Məbləği',
  ...
  en: {
    ...
    licenses: 'Licence Fees', services: 'Service Fees', monthlyService: 'Monthly Service', vat: 'VAT', amountWithVat: 'Total incl. VAT', offerTotal: 'Proposal Total',
...
export class LogoKalemPdfService {
  ...
  private investmentSummary(detail: LogoKalemDetail, lang: Language, t: typeof copy[Language]): string {
    ...
    return `<table class="investment-table"><thead><tr><th>${t.pricing}</th><th>${t.listTotal}</th><th>${t.discountAmount}</th><th>${t.netTotal}</th><th>${t.vat}</th><th>${t.amountWithVat}</th></tr></thead><tbody>${body}</tbody></table><div class="investment-total"><span>${t.offerTotal}</span><strong>${this.amount(total.toFixed(2), currency, lang)}</strong></div><div class="recurring-note"><strong>${t.monthlyService}: ${this.amount(detail.revision.maintenanceTotal, this.currency(detail, ['MAINTENANCE']), lang)}</strong><strong>${t.lem}: ${this.amount(detail.revision.lemTotal, this.currency(detail, ['LEM']), lang)}</strong><p>${this.e(t.recurringExcluded)}</p></div>`;
  }
